...
import {
    Home, Wrench, DollarSign, HelpCircle, Bell, Settings,
    LogOut, Menu, Search, User, ChevronDown, LayoutDashboard, FileText, CreditCard
} from "lucide-react";
                                ...
                                {complaints.map((c, index) => (
                                    <div key={c.id || index} className="card" style={{ margin: 0, padding: "1.5rem" }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                            ...
                                            <span style={{ padding: "4px 8px", borderRadius: "4px", fontSize: "0.8rem", backgroundColor: safeStatus(c.status) === 'OPEN' ? '#FED7D7' : '#C6F6D5', color: safeStatus(c.status) === 'OPEN' ? '#C53030' : '#2F855A' }}>
                                                {safeStatus(c.status)}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    {payments.length === 0 ? (
                                        ...
                                        payments.map(p => (
                                            <tr key={p.id} style={{ borderBottom: "1px solid #E2E8F0" }}>
                                                ...
                                                <td style={{ padding: "12px 20px" }}><span style={{ color: p.status === 'completed' ? 'var(--success-color)' : 'var(--error-color)', fontWeight: "bold" }}>{p.status.toUpperCase()}</span></td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
                ...
                return (
                    <motion.div variants={itemVariants} initial="hidden" animate="visible" className="card">
                        ...
                        <div style={{ marginTop: "20px" }}>
                            <h4>Security HQ</h4><p>📞 0801-000-0000</p>
                            <h4>System Support</h4><p>📞 0801-111-1111</p>
                        </div>
                    </motion.div>
                );
            default:
                        ...
                        <AnimatePresence>
                            {isProfileOpen && (
                                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="profile-dropdown">
                                    ...
                                    <div className="dropdown-item" onClick={() => { setIsChangingPassword(true); setIsEditingProfile(false); setIsProfileOpen(false); }}><Wrench size={16} style={{ marginRight: "8px" }} /> Change Password</div>
                                    <div className="dropdown-item" style={{ color: "var(--error-color)", borderTop: "1px solid #edf2f7" }} onClick={() => { localStorage.clear(); window.location.href = '/login'; }}><LogOut size={16} style={{ marginRight: "8px" }} /> Logout</div>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
