...
const ResidentDashboard = () => {
    ...
    const handleSubmitComplaint = async (e) => {
        ...
        try {
            const res = await fetch(`${API}/api/options/complaints/lodge`, {
                method: "POST",
                headers: {
    ...
    const fetchDues = async (id) => {
        ...
        const token = localStorage.getItem("token");
        try {
            const res = await fetch(`${API}/api/payments/dues/${id}`, {
    ...
    const generateQR = async () => {
        const token = localStorage.getItem("token");
        try {
            const res = await fetch(`${API}/api/visitors/generate`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
    ...
    const renderContent = () => {
        ...
        switch (activeSection) {
            ...
            case "payments":
                return (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card" style={{ maxWidth: "500px" }}>
                        <h2>Make Payments</h2>
            ...
            case "complaints":
                return (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card" style={{ maxWidth: "500px" }}>
                        ...
                        <div className="form-group">
                            <textarea placeholder="Describe the issue..." style={{ minHeight: "100px" }} value={complaintText} onChange={(e) => setComplaintText(e.target.value)}></textarea>
                        </div>
                        ...
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} style={{ width: "100%", backgroundColor: "var(--accent-color)" }} onClick={() => setIsChatbotOpen(true)}>Chat with Assistant</motion.button>
                    </motion.div>
                );
            case "help":
                return (
        ...
        <div className="dashboard-container">
            ...
            <div className="mobile-bottom-nav" style={{ display: 'none' }}>
                {sections.slice(0, 4).map(s => (
                    <div key={s.id} className={`mobile-nav-item ${activeSection === s.id ? "active" : ""}`} onClick={() => setActiveSection(s.id)}>
                        {s.icon}
